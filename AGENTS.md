...
| F9 | **ADX calculation divides by `atr_w` without a zero guard.** In `_add_adx_feature()`, `di_plus` and `di_minus` are computed as `100 * ... / atr_w`. If `atr_w` is zero (e.g. constant prices during a trading halt or very early sparse data), this produces `Inf` or `NaN` that propagates into the `adx_14` feature. The downstream `fillna(0)` on the `dx` step does not protect against this. Fix: add `.where(atr_w > 0, 0.0)` to the `di_plus`/`di_minus` calculations so zero-ATR periods produce `di=0` rather than `Inf`. | `data/features.py` | — | open |
