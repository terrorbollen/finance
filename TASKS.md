...
| B9 | Equity curve CSV export via CLI flag (`--export-equity`): backtester builds equity curves internally but there's no way to get them out | `backtesting/results.py`, `main.py` | — | done |
...
| B17 | **Default backtest horizons don't match model's trained horizons.** `run_backtest()` defaults to `horizons = [1, 2, 3, 4, 5, 6, 7]` when the parameter is `None`. A model trained with `prediction_horizons=[5, 10, 20]` then produces `HorizonPrediction` rows for holding periods 1–4 and 6–7 that it was never trained to predict, making those per-horizon rows in the report meaningless. Fix: change the default to `self.prediction_horizons or [5, 10, 20]` so the backtest analysis horizons always match what the model was trained on. | `backtesting/backtester.py` | — | done |
| B15 | **`backtesting/plot.py` imports from `signals/` — violates INVARIANTS.md module isolation.** `plot.py` does `from signals.direction import Direction` to map signal directions to chart colours. INVARIANTS.md forbids any import between `backtesting/` and `signals/`. Fix: move the `Direction` enum (or a standalone colour-mapping dict) to a shared location (e.g. `common/direction.py`) so both modules can import it without depending on each other, or inline the three colour constants directly in `plot.py`. | `backtesting/plot.py` | — | done |
| B18 | **`backtesting/results.py` imports `Direction` from `signals/` — root cause of the B15 invariant violation.** Line 7: `from signals.direction import Direction as Signal`. Moving this import is the proper long-term fix; it requires relocating `Direction` to a neutral shared module (e.g. `models/direction.py`) and updating all callers: `signals/generator.py`, `models/signal_model.py`, `models/training.py`, `backtesting/results.py`. Discovered during B15. | `backtesting/results.py`, `signals/direction.py`, `signals/generator.py`, `models/signal_model.py`, `models/training.py` | — | done |
| B16 | **Bare `except Exception:` blocks in `backtester.py` swallow real errors silently.** At least two sites use `except Exception:` as a catch-all: one when computing `relative_volume` (falls back to `None`), one when loading calibration buckets (returns `[]`). A genuine bug — e.g. a shape mismatch or file-read error — produces a silent neutral value with no log entry, making it very hard to diagnose. Replace each with the specific exception types that the try block can actually raise (e.g. `(ValueError, IndexError, KeyError)` for volume; `(json.JSONDecodeError, FileNotFoundError, IOError)` for file loading). | `backtesting/backtester.py` | — | done |
...
| R9 | **`DirectionalCalibrator` per-direction fallback path is untested.** When a direction has too few samples to fit its own calibrator, `DirectionalCalibrator` falls back to the global calibrator. This path is never exercised by any test. A regression here (e.g. the fallback silently returning raw confidence) would be invisible in CI. Add a test that constructs a `DirectionalCalibrator` with only one direction fitted and asserts the fallback returns the global calibrator's output. | `tests/test_calibration.py` | — | done |
...
| F5 | Macro indicators (oil prices, interest rates) | `data/features.py`, `data/fetcher.py` | — | done |
...
| M3 | Hyperparameter tuning (systematic search) | `models/training.py` | — | done |
...
| M5 | Ensemble models (LSTM + GRU) — average predictions from both architectures | `models/signal_model.py`, `models/training.py` | — | done |
